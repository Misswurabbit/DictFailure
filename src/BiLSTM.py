...
class BiLSTM(object):
    ...
    def _add_bilstm_cell(self):
        ...
        output, final_state = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_lstm, cell_bw=bw_lstm,
                                                              ...
                                                              initial_state_fw=self.fw_init_state, time_major=False)
    ...
    def _add_output_layer(self):
        ...
        output = tf.reshape(self.bilstm_output, shape=[-1, self.hidden_layer * 2])
        self.output = tf.matmul(output, self.weight['out']) + self.biases['out']

    def _compute_loss(self):
        # compute the loss of the bilstm
        self.loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.output, labels=self.ys)

    def _get_weight(self):
        self.weight = {'in': tf.Variable(tf.random_normal(shape=[self.in_size, self.hidden_layer])),
                       'out': tf.Variable(tf.random_normal(shape=[2 * self.hidden_layer, self.out_size]))
                       }
